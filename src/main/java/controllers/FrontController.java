...
public class FrontController extends HttpServlet {
     ...
     * （例：パラメータがaction=Employeeの場合、action.Employeeオブジェクト
     * @param request リクエスト
    ...
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //リクエストからのパラメータが"action"の値を取得（例："Employee","Report")
            ...
            //該当するActionのオブジェクトにキャスト(例：actions.EmployeeActionオブジェクト→actions.ActionBaseオブジェクト)
            ...
            //リクエストパラメータに設定されている"action"の値が不正の場合（例：action=×××××等、該当するActionクラスがない場合）
